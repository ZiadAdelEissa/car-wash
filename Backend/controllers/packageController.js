...
export const purchasePackage = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Package purchased successfully', userPackage });
  } catch (error) {
    res.status(500).json({ message: 'Package purchase failed', error: error.message });
  }
};
...
export const sharePackage = async (req, res) => {
  try {
    ...
    if (!user.familyMembers.includes(familyMemberId)) {
      return res.status(400).json({ message: 'User is not in your family members' });
    }
    ...
    res.status(500).json({ message: 'Error sharing package', error: error.message });
  }
};
export const createPackage = async (req, res) => {
  ...
  export const deletePackage = async (req, res) => {
    try {
      ...
      res.json({ message: 'Package deactivated successfully', package: deletedPackage });
    } catch (error) {
      res.status(500).json({ message: 'Error deactivating package', error: error.message });
    }
  };